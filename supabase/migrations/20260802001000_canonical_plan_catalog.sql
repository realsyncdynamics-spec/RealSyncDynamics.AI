-- ═══════════════════════════════════════════════════════════════════════════
--  Kanonischer Plan-Katalog (Single Source of Truth in der Datenbank)
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Spiegelt `shared/pricing.ts` als strukturierte Daten in die Datenbank.
-- Der Katalog-Block am Ende dieser Datei wird von
-- `scripts/generate-plan-catalog-sql.ts` erzeugt; der Konsistenztest
-- `test/config/pricing-ssot.test.ts` prüft, dass er zur SSoT passt.
--
-- Additiv: bestehende Tabellen (subscriptions, products, subscription_plans)
-- bleiben unverändert bestehen, RLS-Policies werden nicht angefasst.
--
-- Enthält außerdem die Umbenennung des Plans „scale" → „partner" in allen
-- Bestandsdaten. Der Anwendungscode bildet Altwerte zusätzlich zur Laufzeit
-- über `normalizePlanKey()` ab, damit auch noch nicht migrierte Quellen
-- (z.B. Stripe-Preis-Metadaten) korrekt aufgelöst werden.
...
CREATE TABLE IF NOT EXISTS public.plan_catalog (
  ...
  purchase_mode          TEXT NOT NULL CHECK (purchase_mode IN ('free', 'checkout', 'inquiry')),
  trial_days             INTEGER NOT NULL DEFAULT 0,
  recurring              BOOLEAN NOT NULL DEFAULT false,
  max_bots               INTEGER NOT NULL DEFAULT 0,
  max_answers_per_month  INTEGER NOT NULL DEFAULT 0,
  -- Strukturierte Plandaten. `-1` bedeutet in `limits` durchgaengig unbegrenzt.
...
COMMENT ON TABLE public.plan_catalog IS
  'Kanonischer Plan-Katalog, gespiegelt aus shared/pricing.ts. Keine manuellen Aenderungen.';
...
COMMENT ON TABLE public.plan_addons IS
  'Zentrale Add-on-Definitionen, gespiegelt aus shared/pricing.ts.';
...
-- ─── 2. RLS: oeffentlich lesbar, schreibend nur Service-Role ────────────────
-- Der Katalog enthaelt genau die Informationen, die auch auf /pricing stehen.
...
-- ─── 3. Bestandsdaten: „scale" → „partner" ─────────────────────────────────
-- Reihenfolge beachten: erst die referenzierten Katalogzeilen, dann die
-- referenzierenden Spalten, damit keine FK-Verletzung entsteht.

UPDATE public.subscription_plans SET id = 'partner'
  WHERE id = 'scale' AND NOT EXISTS (SELECT 1 FROM public.subscription_plans WHERE id = 'partner');
UPDATE public.subscription_plans SET id = 'partner_yearly'
  WHERE id = 'scale_yearly' AND NOT EXISTS (SELECT 1 FROM public.subscription_plans WHERE id = 'partner_yearly');
UPDATE public.subscription_plans SET name = replace(name, 'Scale', 'Partner')
  WHERE name LIKE '%Scale%';

UPDATE public.subscriptions SET plan_key = 'partner'        WHERE plan_key = 'scale';
UPDATE public.subscriptions SET plan_key = 'partner_yearly' WHERE plan_key = 'scale_yearly';
UPDATE public.subscriptions SET plan_id  = 'partner'        WHERE plan_id  = 'scale';
UPDATE public.subscriptions SET plan_id  = 'partner_yearly' WHERE plan_id  = 'scale_yearly';

UPDATE public.products SET default_for_plan_key = 'partner'
  WHERE default_for_plan_key = 'scale';
UPDATE public.products SET default_for_plan_key = 'partner_yearly'
  WHERE default_for_plan_key = 'scale_yearly';

-- `free` war historisch ein zweiter Name fuer den Free Audit.
UPDATE public.subscriptions SET plan_key = 'free_audit' WHERE plan_key IN ('free', 'free_tier');
...
UPDATE public.plan_catalog SET active = false, updated_at = now()
  WHERE plan_key NOT IN ('free_audit', 'starter', 'starter_yearly', 'growth', 'growth_yearly', 'agency', 'agency_yearly', 'enterprise', 'enterprise_yearly', 'partner', 'partner_yearly');
