...
-- Note: Enable RLS only on tables that currently exist in schema.
-- Non-existent tables (ai_systems, tenants, profiles, etc.) will be
-- handled in future migrations when those tables are created.

-- ── RLS Policies: Tenant Isolation + Service Role ────────────────
-- ONLY create policies for tables that EXIST in current schema.
-- Pattern: authenticated users see only their tenant_id rows.
-- service_role bypasses RLS (for Edge Functions, Cron, Backfills).

-- ─── governance_approvals (EXISTS in schema) ────────────────────
