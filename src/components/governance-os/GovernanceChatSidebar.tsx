...
//
// Ehrlichkeit (2026-09-25):
//   - Kein Beispiel-„Verlauf" mehr: Die fünf Konversationen dort waren fest
//     eingetragen und ließen sich nicht öffnen.
//     Einen echten Verlauf gibt es in dieser Fläche noch nicht.
//   - Kein „EU-lokal"-Badge: Der Tenant-Chat (`governance-agent`, op 'chat')
//     läuft über Anthropic und routet durch die USA (412, bis bestätigt).
//   - Server-/Netzwerkfehler: ehrliche Meldung + „Erneut versuchen"; der
//     US-Routing-Hinweis wird angezeigt statt die Nachricht still zu
//     verwerfen.
...
export function GovernanceChatSidebar({ open, onClose }: GovernanceChatSidebarProps) {
  ...
  const handleSend = useCallback(() => {
    ...
    if (!text || isLoading || !hasTenant) return;
    void send(text);
    setInputValue('');
    if (textareaRef.current) textareaRef.current.style.height = 'auto';
  }, [hasTenant, inputValue, isLoading, send]);
    ...
    <aside className="hidden lg:flex w-[380px] shrink-0 flex-col bg-obsidian-900 border-l border-titanium-900 h-full overflow-hidden">
      <div className="h-12 shrink-0 flex items-center justify-between px-4 border-b border-titanium-900">
        <div className="flex items-center gap-2">
          <Sparkles className="h-4 w-4 text-[#00B8D4]" />
          <span className="font-mono text-xs font-semibold tracking-widest text-titanium-100 uppercase">RealSync AI</span>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={() => setCommandMenuOpen((v) => !v)} className="p-1.5 text-titanium-500 hover:text-[#00B8D4] hover:bg-obsidian-800 transition-colors" aria-label="Werkzeuge öffnen" title="Werkzeuge">
            <Plus className="h-4 w-4" />
          </button>
          <button onClick={() => void reset()} className="p-1.5 text-titanium-500 hover:text-titanium-200 hover:bg-obsidian-800 transition-colors" aria-label="Neue Konversation" title="Neue Konversation">
          ...
          <div className="grid gap-1 mt-1">
            {COMMAND_ACTIONS.map(({ label, description, icon: Icon, to }) => (
              <button key={label} onClick={() => navigate(to)} className="flex items-center gap-3 rounded-lg px-2.5 py-2.5 text-left hover:bg-obsidian-800 transition-colors">
                <span className="grid h-8 w-8 shrink-0 place-items-center rounded-md bg-[#00B8D4]/10 border border-[#00B8D4]/20"><Icon className="h-4 w-4 text-[#00B8D4]" /></span>
      ...
      <div className="relative shrink-0 border-b border-titanium-900">
        <button onClick={() => setAgentDropdownOpen((v) => !v)} className="w-full h-9 flex items-center justify-between px-4 text-xs text-titanium-200 hover:bg-obsidian-800 transition-colors">
          <div className="flex items-center gap-2">
            <ShieldCheck className="h-3.5 w-3.5 text-[#00B8D4]" />
        ...
        {agentDropdownOpen && (
          <div className="absolute top-full left-0 right-0 z-50 bg-obsidian-950 border border-titanium-900 shadow-xl max-h-64 overflow-y-auto">
            {GOVERNANCE_AGENTS.map((agent) => (
              <button key={agent.id} onClick={() => { setSelectedAgent(agent.id); setAgentDropdownOpen(false); }} className={`w-full flex items-center gap-2 px-4 py-2 text-xs text-left transition-colors hover:bg-obsidian-800 ${agent.id === selectedAgent ? 'text-[#00B8D4] bg-obsidian-800' : 'text-titanium-200'}`}>
      ...
      {usRoutingRequired && <UsRoutingBanner onAck={() => void acknowledgeUsRouting()} />}

      {!hasTenant && (
        <div className="shrink-0 border-b border-titanium-900 px-4 py-2.5 text-[11px] text-titanium-400" data-testid="assistant-no-tenant">
          Kein aktiver Workspace — der Assistent braucht einen Mandanten, um zu antworten.
        </div>
      )}

      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 min-h-0">
        {messages.map((msg) => <ChatMessageView key={msg.id} message={msg} />)}
        {canRetry && <AssistantRetry onRetry={() => void retry()} />}
        {isLoading && (
          <div className="flex items-start gap-2">
            <div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center bg-[#00B8D4] text-[10px] font-bold text-[#1a1917] font-mono">RS</div>
            <div className="border border-white/10 bg-white/5 px-3 py-2"><div className="flex h-4 items-center gap-1">{[0, 1, 2].map((i) => <span key={i} className="h-1.5 w-1.5 animate-bounce bg-[#00B8D4]" style={{ animationDelay: `${i * 150}ms` }} />)}</div></div>
          </div>
        )}
        <div ref={bottomRef} />
      </div>
      ...
      <div className="shrink-0 border-t border-titanium-900 px-4 py-2.5 bg-obsidian-950">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="bg-obsidian-800 border border-titanium-800 px-2 py-0.5 text-[10px] font-mono text-[#00B8D4]">{context.label}</span>
        </div>
        <div className="flex flex-wrap gap-1.5">
          {context.quickActions.map((action) => (
            <button key={action} onClick={() => handleQuickAction(action)} className="px-2.5 py-1 text-[10px] font-mono text-titanium-300 bg-obsidian-800 border border-titanium-800 hover:border-[#00B8D4]/60 hover:text-[#00B8D4] transition-colors">{action}</button>
          ))}
        </div>
      </div>

      <div className="shrink-0 border-t border-titanium-900 px-4 pt-3 pb-3 bg-obsidian-950">
        <div className="rounded-2xl border border-titanium-800 bg-obsidian-900 focus-within:border-[#00B8D4]/60 focus-within:ring-1 focus-within:ring-[#00B8D4]/20 transition-colors overflow-hidden">
          ...
          <div className="flex items-center justify-between px-2 pb-2">
            <button onClick={() => setCommandMenuOpen((v) => !v)} className="grid h-8 w-8 place-items-center rounded-full text-titanium-500 hover:text-[#00B8D4] hover:bg-obsidian-800 transition-colors" aria-label="Tools hinzufügen" title="Tools"><Plus className="h-4 w-4" /></button>
            <div className="flex items-center gap-1.5"><button onClick={handleSend} disabled={!inputValue.trim() || isLoading || !hasTenant} className="grid h-8 w-8 place-items-center rounded-full bg-[#1E5AFF] text-white hover:bg-[#1641C4] disabled:opacity-30 disabled:cursor-not-allowed transition-colors" aria-label="Senden"><Send className="h-3.5 w-3.5" /></button></div>
          </div>
        </div>
