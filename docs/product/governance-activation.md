...
**Status:** P0/P1 product module · first UI slice (draft)  
...
2. GitHub issue summarizing P0/P1 scope
3. In-app Wizard shell as a Governance OS module (`/app/activation`):
   - Organization step + Scope step UI
   - Blueprint placeholder (**Preview**)
   - Document dropzone (**Preview** — no real extraction)
   - Expert Review Queue empty/preview state
   - Continue Activation CTA
   - Native inside `GovernanceBrowserShell` (OS chrome)
4. Optional public landing **section** with §12 copy (does not replace `/` hero)

## Out of scope (this PR)

- Real AI extraction / mapping / 3D graph / event pipeline implementation
- Production data-model migrations without need
...
- Preview URL + screenshots of wizard step 01/02 and review-queue empty/preview
