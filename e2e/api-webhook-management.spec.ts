...
test.describe('Webhook Management Features', () => {
  ...
  test('should provide webhook retry management interface', async ({ page }) => {
    ...
    const response = await page.goto(webhookRetryUrl, { waitUntil: 'networkidle' });
  ...
  test('should display webhook delivery status indicators', async ({ page }) => {
    const response = await page.goto('https://realsyncdynamics-ai.de/app/api/webhook-retry', { waitUntil: 'networkidle' });
  ...
  test('should provide webhook tester interface', async ({ page }) => {
    ...
    const response = await page.goto(webhookTesterUrl, { waitUntil: 'networkidle' });
  ...
  test('should load webhook tester page without errors', async ({ page }) => {
    const response = await page.goto('https://realsyncdynamics-ai.de/app/api/webhook-tester', { waitUntil: 'networkidle' });
    expect(response?.status()).toBeLessThan(500);
  });
  ...
  test('should have functional webhook management routes', async ({ page }) => {
    ...
    let response = await page.goto('https://realsyncdynamics-ai.de/app/api/webhook-retry', { waitUntil: 'domcontentloaded' });
    expect(response?.ok() || response?.status() === 302).toBeTruthy(); // 302 for redirect is acceptable

    // Test webhook tester route
    response = await page.goto('https://realsyncdynamics-ai.de/app/api/webhook-tester', { waitUntil: 'domcontentloaded' });
...
test.describe('Webhook Management Error Handling', () => {
  test('should not return server errors on webhook pages', async ({ page }) => {
    let response = await page.goto('https://realsyncdynamics-ai.de/app/api/webhook-tester', { waitUntil: 'networkidle' });
    expect(response?.status()).toBeLessThan(500);

    response = await page.goto('https://realsyncdynamics-ai.de/app/api/webhook-retry', { waitUntil: 'networkidle' });
    expect(response?.status()).toBeLessThan(500);
  });
});
...
test.describe('Webhook Management UI Responsiveness', () => {
  test('should load webhook management pages on various viewports', async ({ page }) => {
    ...
    response = await page.goto('https://realsyncdynamics-ai.de/app/api/webhook-retry', { waitUntil: 'domcontentloaded' });
    expect(response?.status()).toBeLessThan(500);
  });
});
