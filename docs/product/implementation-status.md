...
4. Interactive Governance Sphere stays off public `/`. Hero visual is photoreal
   ...
   Europe on the limb (UK/FR/DE/IT city lights) + gold route network — no Sphere
   HUD, pointer-events-none. Live H1 (locked words): AI Compliance / Operations
   OS for / Europe (gold last line). Fold: eyebrow · H1 · DISCOVER loop · runtime
   + EU lines · Free Audit starten + Live Dashboard ansehen. Header: Produkt ·
   Evidence · Preise · Login. Assistent chip hidden on `/`.
