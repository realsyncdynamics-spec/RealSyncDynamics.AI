-- ═══════════════════════════════════════════════════════════════════════════
--  Kanonischer Plan-Katalog — Neuausgabe: AP2, drei Self-Service-Stufen
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Ersetzt die Katalog-Daten aus 20260828020000_canonical_plan_catalog.sql.
...
--
-- Neu gegenüber 20260828020000 — AP2, freigegeben am 2026-08-24:
--
--   Starter   `policy_engine` in `modules`. Die Feature-Liste sagte „Policy
--             Packs: DSGVO und EU AI Act" schon vorher zu; die Berechtigung
--             kommt in 20260831000000_ap2_package_model.
--
--   Growth    `api`, `webhooks`, `scheduler`, `bulkOperations` und
--             `provenanceSigning` auf `true`; `apiCallsPerMonth` 0 → 5.000
--             (der Wert, der in `limit.api_calls_monthly` längst stand),
--             `bulkJobsPerMonth` 0 → 10, `apiKeys` 0 → 3. Das ist das neue
--             Zuhause der Fähigkeiten, die bisher erst ab Agency lagen.
--             `whatsapp` fällt aus `addons` — der Kanal ist im Plan
--             enthalten; `voice` und `white_label` kommen dazu, weil sie
--             sonst mit Agency aus dem Angebot fielen.
--
--   Enterprise `purchase_mode` 'checkout' → 'inquiry'. Enterprise ist ab
--             sofort ein Vertrag, kein Self-Service-Checkout. Bestehende
--             Enterprise-Abos rechnen unverändert weiter ab — `stripe-webhook`
--             und die Auflösung über `products` sind davon unberührt.
--             `whatsapp` fällt wie bei Growth aus `addons`.
--
--   Add-ons   `available_for` nennt nur noch Pläne, die verkauft werden.
--             WhatsApp steht jetzt auf `starter` statt auf `growth` und
--             darüber — die Korrektur aus zielzustand-paketmodell.md §3.2:
--             Das Add-on war für den einzigen Plan ohne WhatsApp nicht
--             buchbar und wurde denen angeboten, die ihn schon hatten.
--
-- Agency und Partner bleiben als Zeilen bestehen und werden in
-- 20260831000000_ap2_package_model auf `active = false` gesetzt. Der Block
-- unten fasst sie nicht an: Sie stehen weiterhin in `allPlanKeys()`, damit
-- `planRank()` und `isUpgrade()` für Bestandskunden richtig antworten, und
-- `ON CONFLICT DO UPDATE` schreibt `active` nicht — die Stilllegung überlebt
-- diesen Lauf also.
--
