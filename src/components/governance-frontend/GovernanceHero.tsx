...
function CardLabel({ children, tone = 'cyan' }: { children: ReactNode; tone?: 'cyan' | 'emerald' }) {
  return (
    ...
    >
      <span className={`h-1.5 w-1.5 rounded-full ${tone === 'emerald' ? 'bg-emerald-400' : 'bg-cyan-400'}`} />
      {children}
    </div>
  );
}
...
export default function GovernanceHero({ onStart, backgroundImage }: HeroSectionProps) {
  ...
  return (
    <section id="produkt" className="relative isolate min-h-[780px] overflow-hidden bg-[#02050a] text-white">
      <SpaceBackdrop backgroundImage={backgroundImage} />
      ...
      <header className="relative z-30 mx-auto flex max-w-7xl items-center justify-between px-5 py-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-7 text-sm text-slate-300 lg:flex" aria-label="Hauptnavigation">
          {navItems.map((item) => (
            <a
              key={item.label}
              href={item.href}
              onClick={(e) => handleNav(e, item.href, item.fallback)}
        ...
        <div className="hidden items-center gap-3 lg:flex">
          <a href={routes.login} className="rounded-lg px-4 py-2 text-sm text-slate-300 transition hover:text-white">
            Login
          </a>
      ...
      {menuOpen && (
        <div className="relative z-30 mx-5 rounded-xl border border-white/10 bg-[#071320]/95 p-4 backdrop-blur-xl lg:hidden">
          <nav className="flex flex-col gap-1" aria-label="Mobile Navigation">
            {navItems.map((item) => (
              <a
                ...
                href={item.href}
                onClick={(e) => {
                  handleNav(e, item.href, item.fallback);
                  setMenuOpen(false);
                }}
            ...
            <a href={routes.login} className="rounded-lg px-3 py-2.5 text-sm text-slate-200 hover:bg-white/5">
              Login
            </a>
      ...
      <div className="relative z-20 mx-auto max-w-7xl px-5 pb-24 pt-16 lg:px-8 lg:pt-20">
        <div className="mx-auto max-w-4xl text-center">
          <div className="mb-7 inline-flex items-center gap-2 rounded-full border border-cyan-400/20 bg-cyan-400/5 px-4 py-2 text-xs font-medium tracking-wide text-cyan-200">
            <Sparkles className="h-3.5 w-3.5" />
            AI Governance Runtime
          </div>

          <h1 className="text-balance text-5xl font-semibold leading-[0.98] tracking-[-0.04em] sm:text-6xl lg:text-7xl xl:text-[82px]">
            Das KI-Betriebssystem
            <br />
            für <span className="text-cyan-300">DSGVO, EU AI Act</span>
            <br />
            <span className="text-cyan-300">&amp; Code-Compliance</span>
          </h1>
          ...
          <div className="mt-9 flex flex-col items-center justify-center gap-3 sm:flex-row">
            ...
            <a
              ...
              className="inline-flex h-12 items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-5 text-sm font-medium text-slate-200 transition hover:border-white/20 hover:bg-white/10"
            >
        ...
        <div className="relative mx-auto mt-20 min-h-[300px] max-w-5xl">
          <CardShell className="absolute left-0 top-8 hidden w-56 sm:block">
            <CardLabel>GDPR</CardLabel>
            <div className="mt-2 flex items-end justify-between">
              <span className="text-sm font-medium text-white">Compliant</span>
              <BadgeCheck className="h-5 w-5 text-emerald-300" />
            </div>
          </CardShell>

          <CardShell className="absolute right-0 top-0 hidden w-56 sm:block">
          ...
          <CardShell className="absolute bottom-0 left-1/2 hidden w-64 -translate-x-1/2 sm:block">
            <CardLabel>Claude Code Audit</CardLabel>
            <div className="mt-2 flex items-center justify-between">
              <div>
                <span className="text-2xl font-semibold text-white">94.2%</span>
                <div className="text-[11px] text-slate-400">Code-Ready</div>
              </div>
              <Code2 className="h-7 w-7 text-cyan-300" />
            </div>
          </CardShell>

          <div className="mx-auto flex max-w-3xl flex-wrap items-center justify-center gap-3 pt-8 sm:hidden">
            {highlights.map((item) => {
              const Icon = item.icon;
              return (
                <div key={item.title} className="w-full rounded-xl border border-white/10 bg-[#071320]/80 p-4 text-left backdrop-blur-md">
        ...
        <div className="mt-8 flex justify-center">
          <a
            href={routes.claudeCodeOptimizer}
            onClick={(e: MouseEvent<HTMLAnchorElement>) => handleAnchorClick(e, routes.claudeCodeOptimizer)}
            className="group inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] px-4 py-2 text-xs text-slate-400 transition hover:border-cyan-400/30 hover:text-cyan-200"
          >
