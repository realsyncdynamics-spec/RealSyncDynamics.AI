      ...
      <div className="relative shrink-0 border-b border-titanium-900">
        <button onClick={() => setAgentDropdownOpen((v) => !v)} className="w-full h-9 flex items-center justify-between px-4 text-xs text-titanium-200 hover:bg-obsidian-800 transition-colors">
          <div className="flex items-center gap-2">
            ...
            <span className="flex items-center gap-1"><span className="h-1.5 w-1.5 bg-[#00B8D4] block" /><span className="font-mono text-[10px] text-[#00B8D4]">EU-lokal</span></span>
          </div>
        ...
        {!historyCollapsed && (
          <div>
            {MOCK_CONVERSATIONS.map((conv) => (
              <button key={conv.id} onClick={() => setActiveConversationId(conv.id)} className={`w-full flex items-start gap-3 px-4 py-2 text-left transition-colors hover:bg-obsidian-800 ${activeConversationId === conv.id ? 'bg-obsidian-800 border-l-2 border-[#00B8D4]' : ''}`}>
            ...
            <button onClick={() => setActiveConversationId(null)} className="w-full flex items-center gap-2 px-4 py-2.5 text-xs text-[#00B8D4] hover:bg-obsidian-800 transition-colors border-t border-titanium-900"><Plus className="h-3.5 w-3.5" /><span className="font-mono">Neue Konversation</span></button>
          </div>
        )}
      </div>
