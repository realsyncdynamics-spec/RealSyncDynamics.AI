...
def test_request_size_limit_enforced(app_with_security):
    """Request Size Limit blockiert zu große Requests."""
    client = TestClient(app_with_security)
...
def test_additional_security_headers(app_with_security):
    """Zusätzliche moderne Security Headers sind gesetzt."""
    client = TestClient(app_with_security)
    response = client.get("/api/test")
    ...
    # Cache-Control Header (verhindert Caching sensitiver Daten)
