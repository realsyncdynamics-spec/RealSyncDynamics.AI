...
| 15 | [`ai-router-economics-rfc.md`](./ai-router-economics-rfc.md) | AI-Router & Economics RFC | Layer 2 | `proposed` | Phase-0-Audit des bestehenden AI-/Kostenpfads plus Zielarchitektur für Multi-Provider-Routing mit Kostenkontrolle. Verifizierte Befunde: drei parallele AI-Pfade, von denen nur `_shared/ai.ts` verrechnet wird; `ai-gateway` ist tenant-los; `governance-router` schreibt `cost_usd: 0`; Starter hat keine AI-Entitlements und fällt auf den 250-USD-Default-Cap zurück; `enterprise` steht auf `-1` (unbegrenzt); keine zentrale Preistabelle; xAI/Grok existiert nicht als Provider. Legt fest: AI Units sind Darstellungsform von `limit.ai_cost_monthly_cents`, keine neue Währung — und Security vor Router vor Monetarisierung. |
