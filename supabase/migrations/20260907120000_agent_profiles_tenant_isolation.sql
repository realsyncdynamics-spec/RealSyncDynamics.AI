...
--                      verbucht. `db push` haette diese Datei fuer erledigt
--                      gehalten und STILL uebersprungen — die Lesepolicy unten
...
-- bricht. Eine nur ZU NIEDRIGE Nummer laesst `db push` dagegen laut abbrechen
-- („found local migration files to be inserted before the last migration on
-- remote database"), und zwar fuer ALLE Migrationen, nicht nur fuer diese.
--
-- Die Lehre: Eine Migrationsnummer altert. Sie gehoert vor jedem Merge neu
-- geprueft — gegen `main` UND gegen `supabase_migrations.schema_migrations`.
-- Gegen die doppelte Vergabe schuetzt seit dem 2026-09-02
-- `test/db/migration-versions.test.ts`; gegen das Altern schuetzt nichts
-- ausser Nachsehen.
