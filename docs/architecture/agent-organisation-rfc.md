...
lokalen Baum konnte sie nicht sehen. Die Lehre ist dieselbe wie in CLAUDE.md §5,
nur eine Ebene tiefer: Auch der eigene Checkout ist eine Messung mit Datum. Wer
über den Stand von `main` etwas behauptet, misst gegen `origin/main`.
...
2. **GitHub zeigt es nicht.** Verschiedene Dateien, kein Textkonflikt,
   ...
   wie die Versionskollision vom 2026-08-24 (CLAUDE.md §5) — dort war es eine
   doppelte Versionsnummer, hier ein doppelter Tabellenname.
