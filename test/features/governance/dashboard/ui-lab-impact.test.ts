...
describe('dashboard token impact', () => {
  ...
  it('allows a KPI height inside bounds and lists sister test ids', () => {
    ...
    expect(report.changes[0]?.bindings.every((binding) => binding.kind === 'var')).toBe(true);
  ...
  it('treats a title-size change as low once the clamp override is gone', () => {
    const report = analyzeImpact({ '--dash-title-size': '24px' });
    expect(report.mergeable).toBe(true);
    expect(report.changes[0]?.risk).toBe('low');
    expect(report.changes[0]?.bindings.every((binding) => binding.kind === 'var')).toBe(true);
  });
