    ...
    <main className="min-h-screen bg-[rgb(3,7,18)] text-white antialiased">
      <div className="mx-auto max-w-6xl px-5 py-6 sm:px-8">
        ...
        <section className="mx-auto max-w-5xl py-12">
          ...
          {phase === 'offer' && <div className="mx-auto mt-10 max-w-4xl"><div className="grid gap-4 md:grid-cols-2"><div className="rounded-3xl border border-cyan-400/20 bg-cyan-400/[.04] p-7"><div className="font-mono text-[10px] uppercase tracking-[.18em] text-cyan-300">Website Transformation</div><h2 className="mt-3 text-xl font-bold">DSGVO-/SEO-Umbau durchführen?</h2><p className="mt-2 text-sm text-white/45">Einmaliger Umbau des geprüften Blueprints. Der Preis wird ausschließlich aus der zentralen Pricing-SSoT geladen.</p><div className="mt-6 text-3xl font-bold">{launchTier ? `${launchTier.priceString} €` : 'Preis folgt aus Pricing-SSoT'}</div><div className="text-xs text-white/35">{launchTier?.priceSuffix ?? 'Einmalangebot'}</div><button onClick={() => { setDsgvoUpgrade(true); if (launchTier?.cta.href) window.location.href = launchTier.cta.href; }} disabled={!launchTier} className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-400 px-5 py-3 text-xs font-bold text-[rgb(3,7,18)] disabled:opacity-40">Ja, Umbau starten → Stripe <ArrowRight size={15}/></button></div><div className="rounded-3xl border border-white/10 bg-white/[.03] p-7"><div className="font-mono text-[10px] uppercase tracking-[.18em] text-white/35">Laufende Plattform</div><h2 className="mt-3 text-xl font-bold">Passendes Paket</h2><p className="mt-2 text-sm text-white/45">Die Empfehlung wird aus der bestehenden Pricing-SSoT abgeleitet. Keine Preise oder Planlimits sind hier dupliziert.</p>{planTier && <><div className="mt-6 text-3xl font-bold">{planTier.name}</div><div className="mt-1 text-sm text-cyan-300">{planTier.priceString} € {planTier.priceSuffix}</div><p className="mt-3 text-xs leading-5 text-white/40">{planTier.tagline}</p></>}<button onClick={() => navigate(planTier?.cta.href ?? '/pricing')} disabled={!planTier} className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-white/15 px-5 py-3 text-xs font-bold hover:border-cyan-400/40">{dsgvoUpgrade === false ? 'Nein – normales Paket wählen' : 'Paket ansehen'} <ArrowRight size={15}/></button></div></div><div className="mt-4 text-center text-xs text-white/30">Ja: erst einmaliger Website-Umbau, danach laufendes Paket. Nein: direkt normale Paketauswahl.</div></div>}
