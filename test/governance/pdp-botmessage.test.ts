...
let envFailure: string | undefined;
globalThis.Deno = {
  env: {
    get: (k: string) =>
      k === 'BOT_PDP_ENFORCEMENT' ? envMode
      : k === 'BOT_PDP_FAILURE_MODE' ? envFailure
      : undefined,
  },
};
...
describe('Ausfall des PDP', () => {
  ...
  it('BOT_PDP_FAILURE_MODE=allow lässt bewusst durch, ohne die Durchsetzung abzuschalten', async () => {
    // Der Grund für einen zweiten Schalter: Ohne ihn bliebe einem Betreiber,
    // der Durchsetzung will, aber einen Ausfall nicht als Sperre hinnehmen
    // kann, nur `BOT_PDP_ENFORCEMENT=shadow` — also alles abzuschalten.
    // Dieselbe Trennung wie beim Agent-PEP (AGENT_PDP_FAILURE_MODE, P1-5).
    envMode = 'enforce';
    envFailure = 'allow';
    decideMock.mockRejectedValue(new Error('connection refused'));
    ...
    expect(verdict.allowed).toBe(true);
    expect(verdict.safe_reply).toBeNull();
    // Sichtbar bleiben muss es trotzdem: Ein durchgelassener Ausfall, den
    // niemand im Prüfpfad sieht, ist genau die stille Nicht-Durchsetzung,
    // gegen die dieser Plan geschrieben ist (K1).
    expect(verdict.reasons.join(' ')).toContain('BOT_PDP_FAILURE_MODE=allow');
  });

  it('ohne gesetzten Schalter bleibt es bei fail-closed', async () => {
    // Die Vorgabe darf sich nicht durch Weglassen ändern.
    envMode = 'enforce';
    envFailure = undefined;
    decideMock.mockRejectedValue(new Error('connection refused'));
