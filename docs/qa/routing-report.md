...
| R4 | **Cloudflare-Pages SPA-Deep-Links liefern HTTP 404** (prod + preview), trotz korrektem `dist/_redirects`. `dist/404.html` (GitHub-Pages-Shim) hebelt den `_redirects`-200-Rewrite aus. Browser landen via JS-Redirect dennoch in der App, aber Status = 404. | MITTEL (P2) | **FIX umgesetzt (PR #668):** host-aware Post-Build `scripts/cf-pages-cleanup.mjs` (npm-`postbuild`) entfernt `dist/404.html` **nur** im Cloudflare-Build (`CF_PAGES=1`), sodass `_redirects` `/*  /index.html  200` greift → Deep-Links = HTTP 200. GitHub Pages behält `404.html`. Verifikation per Live-Probe gegen Branch-Preview nach CF-Redeploy. |
| R5 | **Custom-Domain `realsyncdynamicsai.de` liefert HTTP 500** auf allen Pfaden (Edge = Cloudflare, `cf-cache-status: DYNAMIC`, **leerer Body** = Worker-Exception-Signatur). **Root-Cause-Lead:** Der einzige Worker im CF-Account, `realsyncdynamics`, ist ein **Platzhalter-Stub** (`return new Response("Hello world")`). Die Apex-Domain ist offenbar auf einen Stub-/fehlerhaften Worker geroutet statt auf die echte App / das Pages-Projekt. `www.` → 301. | **HOCH (P1) — Produktions-Health** | **Nicht aus dem Repo behebbar** — CF-Dashboard/DNS/Worker-Routing. Apex-Domain auf das Pages-Projekt (`realsyncdynamics-ai`) bzw. den korrekten Worker zeigen lassen; Stub-Worker entfernen/ersetzen. **Nicht durch diesen PR verursacht** (PR-Branch ist nicht auf die Prod-Domain deployt). Vor Go-Live verifizieren. Prod-Infra wurde **nicht** angefasst (nur read-only untersucht). |
