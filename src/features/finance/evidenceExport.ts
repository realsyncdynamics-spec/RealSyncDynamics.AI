// Pure helpers for the Evidence Export Runtime. Build the CSV index +
// JSON manifest that go inside the generated ZIP package. No I/O —
// strings only — so this file is unit-testable from Node-land and the
// Deno Edge Function imports its mirror at
// supabase/functions/_shared/evidenceExport/index.ts. Keep both in sync.

import type {
  TaxDocument, TaxSourceType, TaxEvidenceExport,
} from './types';
...
/**
 * Quote a CSV field iff it contains a delimiter, quote, or newline.
 * Doubles any internal quote — matches the RFC 4180 dialect that
 * Excel + LibreOffice parse correctly.
 */
...
export function buildCsvIndex(documents: TaxDocument[]): string {
  ...
  // Trailing newline keeps tools like `wc -l` accurate.
  return lines.join('\n') + '\n';
}
...
export interface ManifestEntry {
  ...
  /** ZIP-relative path that the corresponding per-document JSON sits at. */
  zip_path: string;
}
...
export interface ExportManifest {
  ...
  tax_year:       number;
  export_id:      string;
  export_type:    TaxEvidenceExport['export_type'];
  generated_at:   string;
...
export function buildManifest(params: {
  tenant_id:   string;
  tax_year:    number;
  export_row:  Pick<TaxEvidenceExport, 'id' | 'export_type'>;
...
// ── README (human-readable cover sheet) ───────────────────────────
...
// ── Per-document JSON (one file per row in the ZIP) ──────────────
