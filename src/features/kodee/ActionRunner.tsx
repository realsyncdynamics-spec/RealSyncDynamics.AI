...
import { Activity, FileText, HardDrive, Globe, Lock, ChevronDown } from 'lucide-react';
        ...
        {ACTIONS.map((def) => {
          ...
          return (
            <div key={def.action} className="relative">
              ...
              {isOpen && hasFields && (
                <div className="absolute bottom-full mb-2 left-0 z-20 bg-white border border-slate-200 rounded-xl shadow-lg p-3 w-64 space-y-2.5">
                  {def.fields!.map((f) => (
                  ...
                  <button
                    ...
                    disabled={busy}
                    onClick={() => fire(def)}
                    className="w-full py-1.5 text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-md disabled:opacity-50"
                  >
