...
| `fix.snippets` | Behebungsvorschläge mit Code | growth, agency, ent, partner | `governance-remediate` (generate) | **ja** (seit AP9 Welle 4, 2026-09-02) | `governance-remediate` → `gateFeature('fix.snippets')` nach owner/admin, nur beim Erzeugen | **`ENFORCED`** | Markieren, Verwerfen, Ersetzen bleiben frei — Bestand darf verwaltet werden | `src/lib/optimizer/entitlement.ts` liest weiterhin `plan.modules` (vierte Vokabular-Ebene, §2) |
| `policy.iso27001` | ISO 27001 als Rahmenwerk | growth, agency, ent, partner | `policy-packs` | **ja** (seit AP9 Welle 4) | `policy-packs` → `policy.packs`, dann je Rahmenwerk des Packs (`frameworks` im Katalog: `ISO_27001` → `policy.iso27001`) beim Aktivieren | **`ENFORCED`** | Deaktivieren bleibt frei | — |
| `policy.nis2` | NIS2 als Rahmenwerk | agency, ent, partner | dito | **ja** (seit AP9 Welle 4) | dito, `NIS2` → `policy.nis2` | **`ENFORCED`** | dito | — |
...
| `alerts.email` | E-Mail-Alert bei neuen Findings | starter … partner | `compliance-alert-trigger`, `audit-monitor-cron` (`email-notify-send` versendet Kontingent-Hinweise, kein Alert) | **ja** (seit AP9 Welle 3, 2026-09-01) | `compliance-alert-trigger` → `hasFeature('alerts.email')` vor jeder E-Mail-Aktion; `audit-monitor-cron` → `mayAlert()` vor dem Versand. Alert und Scan-Ergebnis werden unabhängig vom Plan gespeichert | **`ENFORCED`** | Versand hängt am Key, Prüfpfad nicht | Kein Cron-Job für beide Functions in Produktion (gemessen `cron.job`) — der Versand läuft heute für niemanden |
...
| `governance.risk_register` | Risikoregister | growth, agency, ent, partner | `ai-act-risk-inventory` (`governance-risk-score` gehört zum KI-Register, `governance-risk-escalate` ist intern) | **ja** (seit AP9 Welle 3) | `ai-act-risk-inventory` → `gateFeature('governance.risk_register')` nach der Mitgliedschaft, alle Operationen | **`ENFORCED`** | Derselbe Key wie das Zugriffsregister für `/app/risk-inventory`; `/app/risks` liest per PostgREST, dort gilt nur RLS und das Frontend-Gate | `governance-risk-score` bewusst offen: Neuberechnung aus `/app/assets/:id`, einer freien Fläche |
...
| `whitelabel.reports` | Berichte mit eigenem Logo | agency, ent, partner, Add-on White Label | `tenant-branding-update` | **ja** (seit AP9 Welle 3) | `requireUser` → Rolle owner/admin → `gateFeature('whitelabel.reports')` | **`ENFORCED`** | Vorher fragte die Function einen JWT-Claim ab, den niemand setzt — sie war für alle kaputt (401), siehe `addon-booking.md` §1.6 | — |
| `whitelabel.dashboard` | Eigenes Branding im Dashboard | ent, partner, Add-on White Label | dito | **ja, mittelbar** | derselbe Schreibpfad; `whitelabel.dashboard` ⊆ `whitelabel.reports` (`ENTITLEMENT_DEPENDENCIES`) | **`ENFORCED`** | Wie `bots.chat`: gegated über den Oberbegriff, der Key prüft sich nicht selbst | Eigenes Gate erst, wenn das Dashboard-Theming einen eigenen Schreibpfad hat |
...
| `ENFORCED` seit AP9 (Welle 1–4) | **13** | `bots.appointments`, `bots.orders`, `bots.multi_channel`, `webhooks.enabled`, `alerts.email`, `governance.risk_register`, `whitelabel.reports`, `whitelabel.dashboard` (mittelbar), `fix.snippets`, `policy.iso27001`, `policy.nis2`, dazu `policy.packs`, `ai.tool.workflows` als Bestand — Nachtrag 2026-09-02, die Zeilen oben tragen den jeweiligen Stand. Welle 4 hat außerdem `api.access` und `compliance.export` an den Export-, Berichts- und API-Functions verdrahtet (`reports.export` gewährt kein Plan — nur das Add-on Compliance Pack) (`addon-booking.md` §1.6) |
| `METERED` | **0** | — |
| `DISPLAY_ONLY` | **7** → **2** | ursprünglich `bots.appointments`, `bots.orders`, `dse.generator`, `fix.snippets`, `policy.iso27001`, `policy.nis2`, `sla.priority`; nach AP9 Welle 1–4 bleiben **`dse.generator`** (Frontend-Generator, kein Server-Pfad) und **`sla.priority`** (organisatorische Zusage, siehe §3) |
| `FUNNEL_EXCLUDED` | **0** | — (der einzige Funnel-Fall, `audit-recheck-weekly`, ist keiner dieser Keys) |
| `UNKNOWN` | **7** → **1** | ursprünglich `alerts.email`, `bots.human_handoff`, `bots.multi_channel`, `governance.risk_register`, `webhooks.enabled`, `whitelabel.reports`, `whitelabel.dashboard`; nach AP9 Welle 1–3 bleibt **`bots.human_handoff`** (kein Mechanismus im Repo) |
