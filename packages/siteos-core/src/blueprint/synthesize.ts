// Brief → SiteBlueprint.
// Deterministic SiteOS synthesis. AI may provide structured inputs, but the
// resulting Blueprint remains the canonical deterministic render contract.
...
export interface SynthesizeOptions {
  ...
  /** Transformation source domain. Empty means no external source binding. */
  sourceDomain?: string;
  /** SHA-256 of the deterministic evidence snapshot. */
  evidenceHash?: string;
  /** Canonical visual variant for AI Studio transformations. */
  variant?: SiteBlueprint['transformation']['variant'];
}
...
function buildBlock(
  ...
  switch (kind) {
    case 'navigation': return { ...base, aiGenerated: false, content: { brand: brief.name } };
    case 'hero': return {
      ...base,
      content: {
        headline: brief.name,
    ...
    case 'services': return { ...base, content: { heading: 'Leistungen', items: brief.services.map((label) => ({ label, description: null })) } };
    case 'features': return { ...base, content: {
      heading: 'Warum wir',
      items: [
    ...
    case 'about': return { ...base, content: { heading: 'Über uns', body: brief.summary } };
    case 'team': return { ...base, content: { heading: 'Team', members: [] } };
    case 'testimonials': return { ...base, content: { heading: 'Stimmen', items: [], requiresRealContent: true } };
    case 'faq': return { ...base, content: {
      heading: 'Häufige Fragen',
      items: brief.services.slice(0, 3).map((s) => ({ question: `Was umfasst ${s}?`, answer: null })),
    } };
    case 'contact-form': return { ...base, processesPersonalData: true, content: {
      heading: 'Kontakt', fields: ['name', 'email', 'message'],
    ...
    case 'booking': return { ...base, processesPersonalData: true, content: {
      heading: 'Termin vereinbaren', fields: ['name', 'email', 'phone', 'slot'],
      legalBasis: 'Art. 6 Abs. 1 lit. b DSGVO', privacyHref: '/datenschutz',
    } };
    case 'map': return { ...base, processesPersonalData: true, thirdPartyHosts: [MAP_HOST], content: {
      heading: 'Anfahrt', locality: brief.locality, requiresConsent: true, consentCategory: 'karten',
    } };
    case 'cta': return { ...base, content: { headline: 'Sprechen Sie uns an', href: contactPath(brief) } };
    case 'legal-text': return { ...base, aiGenerated: false, content: { documentRef: legalDocumentRef(path), managed: true } };
    case 'ai-disclosure': return buildAiDisclosureBlock(path, null);
    case 'footer': return { ...base, aiGenerated: false, content: {
      legalLinks: [
...
function buildAiDisclosureBlock(path: string, model: string | null): SiteBlock {
  return {
    ...
    content: {
      ...
      reference: 'Art. 50 EU AI Act', model,
    },
  };
}
...
export function deriveCompliance(
  ...
  const dpiaRequired = presetCompliance.dpiaRequired || (presetCompliance.specialCategories && collectsPersonalData);
  return {
...
export function slugify(input: string): string {
  ...
  const slug = transliterated.replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 64).replace(/-+$/g, '');
...
function contactPath(brief: SiteBrief): string {
  const paths = getIndustryPreset(brief.industry).pagePlan.map((p) => p.path);
...
function pageDescription(title: string, path: string, brief: SiteBrief): string {
  ...
  return brief.locality ? `${title} — ${brief.name} in ${brief.locality}.` : `${title} — ${brief.name}.`;
}
