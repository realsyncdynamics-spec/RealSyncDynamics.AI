        ...
        <div className="lg:col-span-3 space-y-4">
          {workflows.map((wf, idx) => (
            <motion.div 
              ...
              className="bg-obsidian-900 p-0 border border-titanium-900 shadow-sm relative group hover:border-[#0052FF] transition-colors"
            >
              <div className="flex flex-col md:flex-row">
                {/* Left Indicator */}
                <div className={`w-1.5 shrink-0 ${wf.status === 'active' ? 'bg-[#0052FF]' : 'bg-titanium-700'}`}></div>
                
                {/* Content */}
                <div className="flex-1 p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className={`p-2 ${wf.status === 'active' ? 'bg-security-900/30 text-[#0052FF]' : 'bg-obsidian-950 text-titanium-500'}`}>
                        <GitMerge className="h-5 w-5" />
                      </div>
                      <div>
                        <h3 className="text-lg font-bold text-[#0A0A0B] font-mono leading-none mb-1">{wf.name}</h3>
                        <div className="flex items-center gap-2 text-[10px] text-titanium-500 font-bold uppercase tracking-wider">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right hidden sm:block">
                        ...
                        <div className="text-[10px] text-titanium-500 font-bold uppercase">Last: {wf.lastExecution}</div>
                      </div>
                      <button 
                        onClick={() => toggleStatus(wf.id)}
                        className={`p-2 transition-colors border ${wf.status === 'active' ? 'border-red-200 text-red-400 hover:bg-red-950/50' : 'border-emerald-200 text-emerald-400 hover:bg-emerald-950/40'}`}
                  ...
                  <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-titanium-900/50">
                    {wf.actions.map((action, i) => (
                      <React.Fragment key={i}>
                        ...
                        {i < wf.actions.length - 1 && <ArrowRight className="h-3 w-3 text-titanium-600" />}
                      </React.Fragment>
                    ))}
                  </div>
                </div>
          ...
          {isCreating && (
            <motion.div 
              ...
              className="border-2 border-dashed border-titanium-800 p-12 flex flex-col items-center justify-center text-center bg-obsidian-950"
            >
              <Shield className="h-10 w-10 text-titanium-600 mb-4" />
              <h4 className="text-lg font-bold text-titanium-50 mb-2">Editor Initialisierung...</h4>
              <p className="text-sm text-titanium-400 max-w-sm">Wähle einen Trigger (Webhooks, eIDAS Zertifikate oder Ordner-Watcher) um die Automatisierungskette zu starten.</p>
              <button 
        ...
        <div className="space-y-6">
          <div className="bg-[#0A0A0B] p-6 text-white border-l-4 border-[#0052FF]">
             ...
             <div className="space-y-4">
               <div>
                 <div className="flex justify-between text-[10px] font-bold text-titanium-400 mb-1 tracking-wider uppercase">Uptime Monitor</div>
                 <div className="text-2xl font-mono font-bold">99.98%</div>
               </div>
               <div>
                 <div className="flex justify-between text-[10px] font-bold text-titanium-400 mb-1 tracking-wider uppercase">Zertifizierungen (24h)</div>
                 <div className="text-2xl font-mono font-bold text-emerald-500">2,408</div>
               </div>
               <div>
                 <div className="flex justify-between text-[10px] font-bold text-titanium-400 mb-1 tracking-wider uppercase">Compliance Alerts</div>
                 <div className="text-2xl font-mono font-bold text-amber-500">0</div>
               </div>
             </div>
          </div>

          <div className="bg-[#E2E2E2] p-6 border border-titanium-800">
             <ShieldAlert className="h-6 w-6 text-[#0052FF] mb-3" />
             <h3 className="text-sm font-bold uppercase mb-2">Revision & Policy</h3>
             <p className="text-[11px] leading-relaxed text-titanium-300 font-medium">
