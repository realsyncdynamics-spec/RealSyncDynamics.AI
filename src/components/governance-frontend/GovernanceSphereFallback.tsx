...
function stateDot(state: SphereNodeState) {
  return state === 'operational' ? 'bg-[var(--rs-success)]' : 'bg-[var(--rs-warning)]';
}
...
export function GovernanceSphereFallback({
  ...
  return (
    ...
    >
      <div
        className="absolute inset-[10%] overflow-hidden rounded-full border border-[var(--rs-border-primary)] shadow-[0_0_60px_rgba(8,11,15,0.5)]"
        aria-hidden="true"
      >
        <picture>
      ...
      <div
        className="absolute inset-[4%] rounded-full border border-[var(--rs-border-subtle)]"
        aria-hidden="true"
      />
          ...
          return (
            <li key={node.id}>
              <button
                ...
                className={`w-full select-none rounded-xl border px-3 py-2.5 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--rs-cyan-primary)]/60 ${
                  active
                    ? 'border-[var(--rs-cyan-primary)]/55 bg-[var(--rs-cyan-primary)]/10 backdrop-blur-md'
                    : 'border-white/10 bg-black/45 backdrop-blur-md hover:border-[var(--rs-titanium-silver)]/40 hover:bg-black/55'
                }`}
              >
                <span className="flex items-center gap-2">
                  <span className={`h-1.5 w-1.5 rounded-full ${stateDot(node.state)}`} />
                  <span className="font-mono text-[9px] tracking-[.16em] text-[var(--rs-cyan-hover)]">
