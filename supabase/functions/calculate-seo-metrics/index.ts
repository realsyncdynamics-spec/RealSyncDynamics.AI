...
interface MarketingMetricsRecord {
  web_visitors?: number;
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const totalAcquisitionCosts = lifecycleData?.reduce(
      (sum: number, record: CustomerLifecycleRecord) => sum + (record.customer_acquisition_cost || 0),
      0,
    ) || 0
    ...
    const totalLifetimeValue = lifecycleData?.reduce(
      (sum: number, record: CustomerLifecycleRecord) => sum + (record.lifetime_value || record.predicted_ltv || 0),
      0,
    ) || 0
    ...
    const periodMetrics = metricsData && metricsData.length > 0
      ? {
          web_visitors: metricsData.reduce((sum: number, m: MarketingMetricsRecord) => sum + (m.web_visitors || 0), 0),
          leads_generated: metricsData.reduce((sum: number, m: MarketingMetricsRecord) => sum + (m.leads_generated || 0), 0),
          trials_started: metricsData.reduce((sum: number, m: MarketingMetricsRecord) => sum + (m.trials_started || 0), 0),
          customers_acquired: metricsData.reduce((sum: number, m: MarketingMetricsRecord) => sum + (m.customers_acquired || 0), 0),
          revenue: metricsData.reduce((sum: number, m: MarketingMetricsRecord) => sum + (m.revenue_generated || 0), 0),
        }
      : {
          web_visitors: 0,
          leads_generated: 0,
    ...
    const churnedCustomers = lifecycleData?.filter((l: CustomerLifecycleRecord) => l.status === 'churned').length || 0
    const churn_rate =
