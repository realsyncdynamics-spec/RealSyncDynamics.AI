...
export function WebsiteTransformationDashboard() {
  ...
  const currentStage = governancePassed ? 5 : hasLanding ? 3 : latestScan ? 1 : 0;
  ...
  return (
    <main className="min-h-screen bg-[#f7f9fc] text-slate-950">
      ...
      <div className="mx-auto max-w-[1440px] px-5 py-7 lg:px-8 lg:py-9">
        <section className="grid gap-7 lg:grid-cols-[.82fr_1.18fr] lg:items-center">
          <div>
            <p className="text-xs font-bold uppercase tracking-[.2em] text-cyan-600">Ihre Website-Transformation</p>
            <h1 className="mt-3 max-w-2xl text-4xl font-semibold tracking-tight sm:text-5xl">{domain}<br /><span className="text-cyan-600">wird zur neuen Landingpage.</span></h1>
            <p className="mt-4 max-w-xl text-base leading-7 text-slate-600">Wir verwandeln die bestehende Präsentationsschicht in eine moderne, performante und governance-geprüfte Website. <strong className="text-slate-900">Backend, API, Authentifizierung und Datenbank bleiben unverändert.</strong></p>
            <div className="mt-6 flex flex-wrap gap-3"><a href="#preview" className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-slate-950/10 hover:bg-slate-800"><Eye size={15} /> Neue Landingpage ansehen</a><a href="#gate" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50"><ShieldCheck size={15} /> Governance Status</a></div>
            <div className="mt-5 flex flex-wrap gap-5 text-xs text-slate-500"><span>✓ Backend preserve_all</span><span>✓ Evidence gebunden</span><span>✓ SiteOS Renderer</span></div>
          </div>
          <div className="overflow-hidden rounded-3xl bg-slate-950 p-5 text-white shadow-2xl sm:p-6">
            <div className="flex items-center justify-between"><span className="text-[10px] font-bold uppercase tracking-[.2em] text-cyan-300">RealSync AI Studio → SiteOS</span><span className="inline-flex items-center gap-1.5 text-xs text-emerald-300"><span className="h-2 w-2 rounded-full bg-emerald-400" /> LIVE</span></div>
            <div className="mt-5 overflow-hidden rounded-2xl border border-white/10 bg-white/[.06]"><div className="flex items-center gap-1 border-b border-white/10 px-3 py-2"><span className="h-2 w-2 rounded-full bg-red-300/60" /><span className="h-2 w-2 rounded-full bg-amber-300/60" /><span className="h-2 w-2 rounded-full bg-emerald-300/60" /><div className="ml-2 flex-1 rounded bg-white/10 px-3 py-1 text-[10px] text-slate-500">{domain}</div></div><div className="min-h-[290px] p-7 text-center sm:min-h-[340px] sm:p-10"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-cyan-300">GENERATED LANDINGPAGE</p><p className="mx-auto mt-4 max-w-xl text-3xl font-semibold tracking-tight">Klarer. Moderner. Konvertierender.</p><p className="mx-auto mt-3 max-w-md text-xs leading-5 text-slate-400">Die echte SiteOS-Preview ist unten direkt eingebettet.</p><a href="#preview" className="mt-6 inline-flex rounded-full bg-cyan-400 px-5 py-2.5 text-xs font-bold text-slate-950">Preview öffnen <ArrowRightIcon /></a></div></div>
          </div>
        </section>

        {/* 2. PIPELINE */}
        <section className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm lg:p-7">
          <SectionHeading eyebrow="Transformation Pipeline" title="Von der Zahlung bis zum Live-Frontend" subtitle="Die Statusanzeige bleibt an reale Daten gebunden." />
          <div className="mt-6 grid gap-2 sm:grid-cols-3 lg:grid-cols-6">{stages.map((stage, index) => { const done = index < currentStage; const active = index === currentStage; return <div key={stage.key} className={`rounded-2xl border p-3 ${done ? 'border-emerald-200 bg-emerald-50/60' : active ? 'border-cyan-200 bg-cyan-50/70' : 'border-slate-100 bg-slate-50'}`}><div className={`flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold ${done ? 'bg-emerald-500 text-white' : active ? 'bg-cyan-500 text-white' : 'bg-white text-slate-400'}`}>{done ? <Check size={14} /> : index + 1}</div><p className="mt-3 text-xs font-semibold">{stage.label}</p><p className="mt-1 text-[10px] leading-4 text-slate-500">{stage.detail}</p></div>; })}</div>
        </section>

        {/* 3. PREVIEW */}
        <section id="preview" className="mt-6 grid gap-6 lg:grid-cols-[1.65fr_.35fr]">
          <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 px-6 py-5"><div><p className="text-[10px] font-bold uppercase tracking-[.2em] text-cyan-600">Website Preview</p><h2 className="mt-1 text-2xl font-semibold">Ihre neue Landingpage</h2><p className="mt-1 text-xs text-slate-500">Version {site?.version ?? '—'} · deterministisch aus dem SiteOS Blueprint gerendert.</p></div><span className="rounded-full bg-violet-50 px-3 py-1.5 text-xs font-semibold text-violet-700">AI Studio / SiteOS</span></div>
            <div className="p-5 sm:p-6">{previewHtml ? <div className="overflow-hidden rounded-2xl border border-slate-200 shadow-inner"><div className="flex items-center gap-1 border-b border-slate-200 bg-slate-50 px-3 py-2"><span className="h-2 w-2 rounded-full bg-slate-300" /><span className="h-2 w-2 rounded-full bg-slate-300" /><span className="h-2 w-2 rounded-full bg-slate-300" /><div className="ml-2 flex-1 rounded bg-white px-3 py-1 text-[10px] text-slate-400">{domain}</div></div><iframe title="Generierte Landingpage" srcDoc={previewHtml} className="h-[620px] w-full bg-white" sandbox="allow-same-origin" /></div> : <div className="flex min-h-[420px] items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 text-center"><div><Globe2 className="mx-auto h-8 w-8 text-cyan-500" /><p className="mt-4 font-semibold">Landingpage wird vorbereitet</p><p className="mt-2 max-w-sm text-xs leading-5 text-slate-500">Der SiteOS-Blueprint ist noch nicht als Preview verfügbar. Aktualisieren Sie nach Abschluss der Generierung.</p></div></div>}
              <div className="mt-4 flex flex-wrap gap-2"><Link to="/app/siteos" className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-xs font-semibold text-white">Im Builder öffnen <ExternalLink size={13} /></Link><button onClick={() => void reload()} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-semibold text-slate-700"><RefreshCw size={13} /> Preview aktualisieren</button></div>
            </div>
          </section>
          <div className="space-y-6">
            <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-[10px] font-bold uppercase tracking-[.2em] text-cyan-600">Varianten</p><h3 className="mt-1 font-semibold">4 Designrichtungen</h3><div className="mt-4 grid grid-cols-2 gap-2">{['Executive', 'Modern', 'Authority', 'Minimal'].map((v, i) => <div key={v} className={`rounded-xl border p-3 ${i === 1 ? 'border-cyan-300 bg-cyan-50' : 'border-slate-100 bg-slate-50'}`}><p className="text-xs font-semibold">{v}</p><p className="mt-1 text-[10px] text-slate-500">Evidence identisch</p></div>)}</div></section>
            <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-[10px] font-bold uppercase tracking-[.2em] text-cyan-600">Project</p><h3 className="mt-1 font-semibold">{domain}</h3><div className="mt-4 space-y-3 text-xs"><InfoRow label="Transformation" value="€349 · bestätigt" /><InfoRow label="Blueprint" value={blueprint ? 'vorhanden' : 'ausstehend'} /><InfoRow label="Backend" value="preserve_all" /><InfoRow label="Version" value={site ? `v${site.version}` : '—'} /></div></section>
          </div>
        </section>

        {/* 4. FINDINGS */}
        <section className="mt-6 grid gap-6 lg:grid-cols-[1fr_1fr]">
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><SectionHeading eyebrow="Audit Evidence" title="Befunde als Wertnachweis" subtitle="Nur tatsächlich vorliegende SiteOS-Werte werden angezeigt — keine erfundenen Verbesserungsdeltas." /><div className="mt-6 grid grid-cols-2 gap-3"><Metric label="Health" value={site?.health} /><Metric label="Compliance" value={site?.compliance} /><Metric label="Performance" value={site?.performance} /><Metric label="Risk" value={site?.risk} inverse /></div><div className="mt-5 rounded-2xl bg-slate-50 p-4 text-xs leading-5 text-slate-500">Letzter Evidence-Lauf: {latestScan ? new Date(latestScan.observed_at).toLocaleString('de-DE') : 'noch nicht vorhanden'}{latestScan ? ` · ${latestScan.findings.length} Findings` : ''}</div></section>
