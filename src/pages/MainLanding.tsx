...
import { ArrowRight, Code2, FileCheck2, Lock, Scan, ShieldCheck } from 'lucide-react';
  ...
  return (
    <div className="landing-context min-h-screen bg-[rgb(3,7,18)] text-white antialiased" style={{ backgroundColor: BG, fontFamily: SANS }}>
      <SEOHead
        ...
        description="Das Governance OS für DSGVO und EU AI Act. Website-Governance, Evidence Vault, Policy Packs, auditfähiger Nachweis-Export und kontinuierliches Monitoring in einer Runtime."
        canonical="/"
      ...
      <main ref={revealRoot}>
          ...
          <div className="relative z-10 mx-auto grid min-h-[880px] max-w-7xl items-center gap-10 px-6 pb-20 pt-28 lg:grid-cols-[1.05fr_.95fr] lg:px-10">
            <div className="max-w-3xl">
              ...
              <p className="mt-5 font-mono text-[11px] uppercase tracking-[.28em] text-white/35">
                <Scan className="mr-2 inline h-3.5 w-3.5" />KI-Governance Runtime für regulierte Unternehmen
              </p>
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                <Link to="/branchen" className="inline-flex items-center gap-2 rounded-full border border-[#e8c98a]/45 px-7 py-3.5 font-medium text-[#fff8ee] transition hover:bg-white/5">Branchen <ArrowRight className="h-4 w-4" /></Link>
                ...
                <Link to="/claude-code-optimizer" className="inline-flex items-center gap-2 px-3 py-3.5 text-sm text-white/45 transition hover:text-white/70">Code-Optimizer</Link>
              </div>
            </div>
        ...
        <section className="border-t border-white/10 bg-black/80 py-20 md:py-28"><div className="mx-auto max-w-3xl px-6 text-center"><p className="font-mono text-[10px] tracking-[.25em] text-[#e8c98a]">WARUM JETZT</p><h2 className="hero-shine mt-4 text-4xl tracking-tight sm:text-6xl" style={{ fontFamily: SERIF, fontWeight: 500 }}>Governance statt Checkliste.</h2><p className="mx-auto mt-5 max-w-2xl text-base leading-relaxed text-[#e8dcc4]">Eine Checkliste beruhigt bis zum nächsten Audit. Die Runtime hält den Nachweis, wenn Aufsicht, Kunde oder Board fragt.</p><div className="mt-8 flex flex-wrap justify-center gap-3 text-xs text-white/50"><span className="rounded-full border border-white/10 px-4 py-2">DSGVO</span><span className="rounded-full border border-white/10 px-4 py-2">EU AI Act</span><span className="rounded-full border border-white/10 px-4 py-2">Policy Packs</span><span className="rounded-full border border-white/10 px-4 py-2">Evidence Vault</span><span className="rounded-full border border-white/10 px-4 py-2">Nachweis-Export</span></div><div className="mt-9 flex flex-col justify-center gap-3 sm:flex-row"><Link to="/pricing" className="inline-flex items-center justify-center gap-2 rounded-full bg-[#f0e6d2] px-7 py-3.5 font-semibold text-[#1a1714] transition hover:bg-[#f6efe4]">Preise ansehen <ArrowRight className="h-4 w-4" /></Link></div></div></section>
      </main>

      <footer className="border-t border-white/10 py-8"><div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-6 text-xs text-white/40 sm:flex-row lg:px-10"><span>© 2026 RealSync Dynamics.AI</span><div className="flex gap-5"><Link to="/branchen">Branchen</Link><Link to="/impressum">Impressum</Link><Link to="/datenschutz">Datenschutz</Link><Link to="/agb">AGB</Link></div></div></footer>
    </div>
  );
}
