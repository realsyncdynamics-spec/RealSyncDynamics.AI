...
export function hasUsageBasedModules(selection: readonly BookableModuleId[]): boolean {
  const ids = new Set<BookableModuleId>(selection);
  for (const module of REQUIRED_MODULES) ids.add(module.id);
  return [...ids].some((id) => { const model = bookableModuleById(id)?.priceModel; return model === 'flat_plus_usage' || model === 'credits'; });
}
...
export function planById(id: PlanId): Plan { const canonicalId = id in LEGACY_PLAN_ID_ALIASES ? LEGACY_PLAN_ID_ALIASES[id as LegacyPlanId] : id as CanonicalPlanId; const plan = PLANS.find((p) => p.id === canonicalId); if (!plan) throw new Error(`[pricing] unbekannte PlanId: ${id}`); return plan; }
...
export function planRank(id: PlanId): number { const canonicalId = id in LEGACY_PLAN_ID_ALIASES ? LEGACY_PLAN_ID_ALIASES[id as LegacyPlanId] : id as CanonicalPlanId; return PLAN_ORDER.indexOf(canonicalId); }
...
export function recommendPlan(input: RecommendationInput): PlanRecommendation {
  ...
  if (needsApi || needsWhiteLabel || domains > planById('growth').limits.domains) return { planId: 'business', reason: 'API, Webhooks, White-Label oder mehr Domains erfordern BUSINESS.' };
