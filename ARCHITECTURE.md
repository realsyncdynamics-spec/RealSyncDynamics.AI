...
Kalibrierung und Freigabevertrag für den Slot `decision-intelligence`:
[`docs/adr/0012-decision-intelligence-calibration.md`](docs/adr/0012-decision-intelligence-calibration.md).

Schwellen, Stichprobengrößen und Drift-Gates leben nur in dieser ADR und werden
versioniert. Kalibrierung ist Evidenz für Provider-Verlässlichkeit, keine
Autorisierung. Autoritätskette: `schema_valid` → `calibration_passed` →
`policy_authorized` → `execution_verified`.
