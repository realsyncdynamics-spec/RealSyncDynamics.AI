...
export default function GlobeVisual() {
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => { const r=el.getBoundingClientRect(); const x=(e.clientX-r.left)/r.width-.5; const y=(e.clientY-r.top)/r.height-.5; el.style.setProperty('--mx',`${x*14}deg`); el.style.setProperty('--my',`${y*-10}deg`); };
  ...
  return <div ref={ref} className="rs-governance-globe" aria-hidden="true">
    ...
    <div className="rs-globe-sphere"><div className="rs-globe-grid rs-grid-a"/><div className="rs-globe-grid rs-grid-b"/><div className="rs-globe-light"/>
      ...
      {links.map(([a,b],i)=>{const p=nodes[a],q=nodes[b];return <line key={i} x1={p.x} y1={p.y} x2={q.x} y2={q.y} className="rs-network-line"/>})}
      {nodes.map((n,i)=><g key={i} className="rs-network-node" style={{animationDelay:`${i*170}ms`}}><circle cx={n.x} cy={n.y} r={n.r*2.8} className="rs-node-pulse"/><circle cx={n.x} cy={n.y} r={n.r} className="rs-node-core"/></g>)}
      </svg>
    </div>
