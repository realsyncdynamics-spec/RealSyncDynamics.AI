...
// Bottom-center on every public page. Tapping it routes to /audit with a
// source-attribution query so the chat flow on the audit page can pick up
// the intent.
//
// Intentionally a plain Link, not a chat modal — the platform's chat UX
// lives on /audit (AuditChatHero) where it can collect URL + email before
// hitting the governance-agent. The chip's only job is to make the
// assistant always reachable.
