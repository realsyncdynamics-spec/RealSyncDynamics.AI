...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === 'GET') {
    if (url.searchParams.get('mode') !== 'waitlist') return jsonError(405, 'METHOD_NOT_ALLOWED', 'POST only', cors);
    const admin = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, { auth: { persistSession: false } });
    const { count, error } = await admin.from('waitlist_signups').select('id', { count: 'exact', head: true }).in('status', ['pending', 'invited', 'converted']);
  ...
  if (body.mode === 'waitlist') {
    const { count: recent } = await admin.from('waitlist_signups').select('id', { count: 'exact', head: true }).eq('ip_hash', ipHash).gte('created_at', oneHourAgo);
    if ((recent ?? 0) >= WAITLIST_RATE_LIMIT) return jsonError(429, 'RATE_LIMITED', 'Zu viele Anmeldungen. Bitte später erneut versuchen.', cors);
    ...
    const { data: existing } = await admin.from('waitlist_signups').select('position').eq('email', email).in('status', ['pending', 'invited', 'converted']).maybeSingle();
    if (existing) return jsonResponse({ ok: true, position: existing.position, already_registered: true }, 200, cors);
    const { data: row, error: wlError } = await admin.from('waitlist_signups').insert({
      email, company: capOrNull(body.company, 160), role: capOrNull(body.role, 120), interest,
      team_size: teamSize, note: capOrNull(body.note, 2000), source: capOrNull(body.source, 64) ?? 'warteliste',
      referrer: capOrNull(body.referrer, 500), utm: pickUtm(body.utm), ip_hash: ipHash,
      user_agent: capOrNull(req.headers.get('user-agent'), 500),
    }).select('position').single();
    if (wlError) {
      if (wlError.code === '23505') {
        const { data: raced } = await admin.from('waitlist_signups').select('position').eq('email', email).in('status', ['pending', 'invited', 'converted']).maybeSingle();
        if (raced) return jsonResponse({ ok: true, position: raced.position, already_registered: true }, 200, cors);
      }
  ...
  const { data, error } = await admin.from('sales_leads').insert({
    name, email, company, use_case: cap(body.use_case, 50), message: cap(body.message, 4000),
    source: cap(body.source, 200), path: cap(body.path, 500), user_agent: cap(req.headers.get('user-agent'), 500),
    ip_hash: ipHash, metadata: { ...(intent ? { intent } : {}), ...(tier ? { tier } : {}), ...(body.source === 'unified-entry' ? { marketing_consent: true } : {}) },
  ...
  if (body.source === 'unified-entry' && intent === 'starter_3_months_free' && tier === 'starter') {
    try { await sendStarterOffer(email, name, company); } catch (emailError) { console.error('starter offer email failed:', emailError); }
  }
