...
| 13 | [`ecosystem-vertical-integration.md`](./ecosystem-vertical-integration.md) | Ökosystem-Zusammenführung | cross | `proposed` | Äußerer Schnitt: GitHub-Org, tote Vercel-Siblings, paralleles Agent-OS, Domain-Bruch. Vertikale Achse Identität → Beobachtung → PDP → Evidence → Ausführung → `/app`. Ausdrücklich kein Monorepo-Merge. |
| 14 | [`agent-organisation-rfc.md`](./agent-organisation-rfc.md) | RFC-005 — Agenten-Organisationsebene: was ADR 0011 offen lässt | Layer 4 | `proposed` | Ergänzung zu ADR 0011, keine konkurrierende Entscheidung. Behandelt vier Punkte, die ADR und PR #1202 nicht abdecken: den ungelösten Widerspruch zu „kein fünftes Agenten-System" aus `agent-manager-roadmap.md`, die Dopplungen mit M0–M4, die verifizierte `org_units`-Kollision zwischen PR #1135 (auf `main`) und PR #1202, die Sprachregelung zu „AGI Manager" und den Betriebsbefund aus der Bestandsaufnahme. §0 korrigiert die erste Fassung. |
| 15 | [`ai-router-economics-rfc.md`](./ai-router-economics-rfc.md) | AI-Router & Economics RFC | Layer 2 | `proposed` | Phase-0-Audit des bestehenden AI-/Kostenpfads plus Zielarchitektur für Multi-Provider-Routing mit Kostenkontrolle. Verifizierte Befunde: **fünf** AI-Pfade, von denen nur `_shared/ai.ts` vollständig verrechnet; `ai-gateway` tenant-los; `governance-router` mit `cost_usd: 0`; `governance-agent` und `optimize-analyze` ohne Reserve/Settle; Starter ohne AI-Entitlements (Fail-Open auf 250-USD-Default); `enterprise: -1`; drei parallele Preisquellen; `override_until` nie gelesen; xAI/Grok kein Provider. Entschieden: Growth und Partner-Bestand bleiben unverändert, `enterprise: -1` wird durch ein explizites Budgetmodell ersetzt. Leitinvariante — kein Provider-Request außerhalb des Cost-Gates — als Contract-Test in `test/contracts/`, nicht als Konvention. |
