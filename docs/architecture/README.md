...
| 13 | [`agent-organisation-rfc.md`](./agent-organisation-rfc.md) | RFC-005 — Organisations- und Rollenebene | Layer 4 | `proposed` | Bewertung eines Entwurfs für eine Rollen-/Organisationsebene über der Agenten-Landschaft (Trennung Rolle ↔ Fähigkeit, Bericht-Kompression, Eskalation als Datensatz). Grenzt ab gegen M0–M4 aus `agent-manager-roadmap.md`, benennt Dopplungen, löst die Namenskollision im `agent_`-Präfix und beantwortet die fünf offenen Fragen des Entwurfs gegen den gemessenen Produktionsstand. Vorbedingungen in §6 — keine Migration vor deren Erfüllung. |
