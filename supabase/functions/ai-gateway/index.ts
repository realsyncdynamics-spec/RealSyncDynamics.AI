...
async function handleOpBased(req: Request): Promise<Response> {
  ...
  // Der Provider-Call ist zusaetzlich von withObservability (R2) umschlossen,
  // damit jeder Call flag-gated in ai_tool_runs landet. Reihenfolge: erst PEP
  // (blockiert, bevor ein Provider-Call Kosten verursacht), dann Inferenz.
  if (op === 'generate')     return jsonResponse({ ok: true, ...(await withObservability(request, () => gateway.generate(request))), ...(governance ? { governance } : {}) });
  if (op === 'extract_json') return jsonResponse({ ok: true, ...(await withObservability(request, () => gateway.extractJson(request))), ...(governance ? { governance } : {}) });
  if (op === 'embed')        return jsonResponse({ ok: true, ...(await withObservability(request, () => gateway.embed(request))), ...(governance ? { governance } : {}) });
