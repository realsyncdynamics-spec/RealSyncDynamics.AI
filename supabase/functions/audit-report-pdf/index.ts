...
function severityMeta(severity: string): { label: string; color: string } {
  return { label: SEV[severity] ?? severity.toUpperCase(), color: severityColor(severity) };
}
function escapeHtml(s: string | null | undefined): string {
  return s?.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;') ?? '';
}

function buildHtml(audit: AuditRow, reportUrl: string): string {
  ...
  const statsHtml = statsByLevel.map(({ level, count }) => {
    const { label, color } = severityMeta(level);
  ...
  const rows = audit.issues.map(issue => {
    const { label, color } = severityMeta(issue.severity);
