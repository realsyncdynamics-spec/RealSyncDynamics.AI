...
- **188 Edge Functions**, im Repo wie in Produktion, **deckungsgleich in beide Richtungen** (`comm -23` und `comm -13` beide leer). Gemessen am **2026-09-06 um 19:56 UTC** per Management-API gegen `ebljyceifhnlzhjfyxup`; alle 188 tragen `status: ACTIVE`. `_shared` ist Bibliothek, keine Function.

  Die sechs, die hier bis zum 2026-09-06 als „warten auf den nächsten `deploy.yml`-Lauf" standen — `governance-decide`, `integration-credentials`, `governance-access`, `evidence-anchor`, `microsoft365-connect`, `microsoft365-audit-sync` —, sind an diesem Tag zwischen **13:39 und 13:41 UTC** deployt worden. `src/config/production-edge-functions.ts` hinkte danach hinterher und hat den `Edge Function Drift Guard` auf **jedem** offenen PR rot gefärbt, ohne dass einer davon die Ursache war. Nachgezogen: Liste auf 188, `EDGE_FUNCTIONS_OBSERVED_MAX` von 182 auf 188, `PRODUCTION_EDGE_FUNCTIONS_MEASURED_AT` auf den Messzeitpunkt.

  > **Korrektur einer Zahl, die hier falsch stand**: Der Satz „Fünf davon stehen in `UNBACKED_CALLERS`" traf nicht zu — es waren **vier** (`integration-credentials`, `governance-access`, `evidence-anchor`, `microsoft365-connect`). `governance-decide` stand dort nie, aus demselben Grund wie `microsoft365-audit-sync`: kein Aufrufer im Frontend. Die Liste führt Aufrufer ohne Backend, nicht Functions ohne Deploy — bei zwei der sechs traf schon die erste Hälfte nicht zu. Am Code nachgezählt, nicht aus der Aufzählung geschlossen; genau der Fehler, vor dem §5 warnt.

  **Die Lehre ist die Betriebsfolge, nicht die Zahl.** Diese Liste ist eine *statische Behauptung über Produktion*. Sie altert bei jedem Deploy — und zwar ohne dass jemand etwas falsch macht. Solange sie hinterherhinkt, blockiert sie fremde PRs, und deren Autoren sehen einen roten Check, den sie nicht verursacht haben und nicht beheben können. **Nach jedem `deploy.yml`-Lauf, der Functions hinzufügt, gehört sie nachgemessen** — nicht erst, wenn jemand darüber stolpert.
