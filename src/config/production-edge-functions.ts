...
export const PRODUCTION_EDGE_FUNCTIONS_MEASURED_AT = '2026-09-06T13:50Z';
...
export const UNBACKED_CALLERS: readonly UnbackedCaller[] = [
  ...
  // Die vier Governance-OS-Aufrufer aus PR #1135 standen hier bis zum
  // 2026-09-06 — `integration-credentials` (P0-1), `governance-access`
  // (P1-3), `evidence-anchor` (P1-6) und `microsoft365-connect` (P2-2).
  // Nach dem Merge und dem gruenen Deploy-Lauf 34036663016 sind sie live
  // gemessen (13:50 UTC, Management-API, 188 = 188, `comm` in beide
  // Richtungen leer) und deshalb entfernt.
  //
  // Zwei Slugs aus demselben PR standen nie hier, aus zwei verschiedenen
  // Gruenden — beide sind es wert, unterschieden zu werden:
  //   * `microsoft365-audit-sync` hat keinen Aufrufer im Frontend, sondern
  //     wird von pg_cron getriggert.
  //   * `governance-decide` wird von Edge Function zu Edge Function
  //     gerufen, nie aus dem Browser.
  // Diese Liste fuehrt Aufrufer OHNE Backend — nicht Functions ohne Deploy
  // und nicht Functions ohne Aufrufer.
];
