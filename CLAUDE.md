...
- **188 Edge Functions** im Repo (`supabase/functions/`, Deno/V8; `_shared` ist Bibliothek, keine Function) — gemessen am 2026-09-05 am **Merge-Baum** (`ls -d`, nicht addiert). **Alle 188 sind deployt**, deckungsgleich in beide Richtungen — gemessen am 2026-09-06 um 15:26 UTC per Management-API im Lauf 34042302978: 188 live, 188 im Repo. Die **sechs**, die hier bis dahin als „warten auf den nächsten `deploy.yml`-Lauf" standen (`governance-decide` und `integration-credentials` P0, `governance-access` P1-3, `evidence-anchor` P1-6, `microsoft365-connect` und `microsoft365-audit-sync` P2-2), sind angekommen.

  > **Der Guard hat es gemeldet, bevor jemand hinsah — und die Meldung landete auf dem falschen PR.** `src/config/production-edge-functions.ts` blieb nach dem Deploy auf 182 Einträgen stehen. Weil der Drift-Guard bei jedem PR läuft, färbte diese Veraltung auf `main` **fremde** PRs rot: #1229 (Landing/Preise) stand mit `STALE_PROD_LIST` da, ohne eine einzige Edge Function anzufassen. Wer nur auf die Ampel des eigenen PRs sieht, sucht den Fehler dann im eigenen Diff. **Ein roter Guard nennt nicht immer den Verursacher — er nennt den Zustand.** Nachgezogen am 2026-09-06; dabei sind die vier Einträge aus `UNBACKED_CALLERS` entfernt worden, die der Kommentar dort selbst zur Entfernung „nach der Neumessung" vorgemerkt hatte.
  >
  > Eine Zahl hier war zusätzlich falsch: Es hieß, **fünf** der sechs stünden in `UNBACKED_CALLERS`. Es waren **vier** — `governance-decide` fehlte dort zu Recht, denn es hat keinen Aufrufer im Frontend, sondern wird von `ai-gateway`, der Agent-Runtime und `platform/…/pdp_client.py` gerufen. Damit gilt für `governance-decide` derselbe Grund, der für `microsoft365-audit-sync` ausdrücklich vermerkt war; nur stand er dort nicht
