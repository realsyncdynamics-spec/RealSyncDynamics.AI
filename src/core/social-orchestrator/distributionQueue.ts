...
// ── Real publishers — TODO ─────────────────────────────────────────
//
// Each follow-up PR adds a single channel adapter. Implementation
// notes:
//
// LinkedIn:
//   - OAuth 2.0 + REST `https://api.linkedin.com/v2/ugcPosts`.
//   - Token in Supabase Vault under `linkedin_access_token`.
//   - Distinct profiles for `linkedin.enterprise` (company page) vs
//     `linkedin.legal` (personal author of the DPO).
//   - Honour 1500-char body cap minus hashtag block.
//
// Meta (Instagram + Threads):
//   - Graph API `/{ig-user-id}/media` + `/media_publish`.
//   - Asset must be uploaded first (image / reel video).
//   - The orchestrator only generates caption text — asset attach
//     happens in the publisher adapter.
//   - Token in Vault under `meta_access_token`.
//
// TikTok:
//   - TikTok for Business API; requires sandbox approval.
//   - Token rotation every 24 h via refresh-token flow.
//
// X:
//   - X API v2 `/2/tweets`. Note: Free-tier write quota.
//   - 280-char hard limit; the post generator already enforces 260.
//
// All adapters MUST:
//   - never log secret token material,
//   - return PublishResult with `externalId` so the queue can
//     correlate later analytics back to the queue entry,
//   - surface 4xx as `ok: false` with error.code matching the
//     upstream code (e.g. `LINKEDIN_429_RATE_LIMIT`).
