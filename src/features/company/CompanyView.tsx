...
export function CompanyView() {
  ...
  if (searchParams.get('view') === 'crm') return <CrmView />;
    ...
    <div className="px-4 sm:px-6 py-8 max-w-3xl mx-auto space-y-8 text-titanium-100">
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {INDUSTRY_PROFILES.map((p) => {
            ...
            return (
              <button key={p.key} onClick={() => update({ industry: p.key })} className={`text-left p-3 border rounded-none transition-colors ${active ? 'border-cyan-400 bg-obsidian-800' : 'border-titanium-800 bg-obsidian-900 hover:border-titanium-600'}`}>
                <div className="font-semibold text-sm text-titanium-50 flex items-center gap-1.5">{active && <CheckCircle2 className="h-3.5 w-3.5 text-cyan-300" />} {p.label}</div>
      ...
      <section className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {COMPANY_SIZES.map((s) => (
            <button key={s} onClick={() => update({ companySize: s })} className={`px-3 py-1.5 text-sm border rounded-none font-mono ${draft.companySize === s ? 'border-cyan-400 bg-obsidian-800 text-titanium-50' : 'border-titanium-800 bg-obsidian-900 text-titanium-400 hover:border-titanium-600'}`}>{s}</button>
          ))}
        </div>
      </section>
        ...
        <>
          <section className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {industry.suggestedTools.map((tool) => (
                <button key={tool} onClick={() => toggleTool(tool)} className={`px-3 py-1.5 text-sm border rounded-none ${draft.usedTools.includes(tool) ? 'border-cyan-400 bg-obsidian-800 text-titanium-50' : 'border-titanium-800 bg-obsidian-900 text-titanium-400 hover:border-titanium-600'}`}>{tool}</button>
              ))}
            </div>
          </section>
          ...
          <section className="space-y-3">
            ...
            <ul className="space-y-2">
              {industry.risks.map((r) => <li key={r} className="flex items-start gap-2 text-sm text-titanium-200 border border-titanium-900 bg-obsidian-900 p-3"><ShieldAlert className="h-4 w-4 text-amber-400 shrink-0 mt-0.5" /> {r}</li>)}
            </ul>
          </section>
          ...
          <section className="space-y-3">
            ...
            <div className="space-y-2">
              {industry.tasks.map((t) => <Link key={t.href + t.label} to={t.href} className="flex items-center justify-between gap-2 p-3 border border-titanium-800 bg-obsidian-900 hover:border-cyan-400 rounded-none group"><span className="text-sm text-titanium-100">{t.label}</span><ArrowRight className="h-4 w-4 text-titanium-500 group-hover:text-cyan-300" /></Link>)}
            </div>
          </section>
        </>
      ) : <p className="text-sm text-titanium-500 border border-titanium-900 bg-obsidian-900 p-4">Wählen Sie oben Ihre Branche, um passende Risiken und nächste Schritte zu sehen.</p>}
    </div>
  );
}