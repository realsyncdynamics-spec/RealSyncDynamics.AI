...
describe('DSGVO-Tool-Landschaft', () => {
  ...
  it('begruendet die Transfer-Stufe konsistent mit dem Anbietersitz', () => {
    ...
    for (const t of DSGVO_TOOLS) {
      if (t.origin === 'EU' && t.name !== 'Cookiebot (Cybot)') {
  ...
  it('zieht Matrix-Wettbewerberpreise und Cookiebot-Transfer aus competitor-pricing', () => {
    expect(toolByName('OneTrust')?.pricingFrom).toBe(COMPETITOR_PRICING.OneTrust.pricing);
    expect(toolByName('Usercentrics')?.pricingFrom).toBe(COMPETITOR_PRICING.Usercentrics.pricing);
    expect(toolByName('Cookiebot (Cybot)')?.pricingFrom).toBe(COMPETITOR_PRICING.Cookiebot.pricing);
    expect(toolByName('Cookiebot (Cybot)')?.transfer).toBe(COMPETITOR_PRICING.Cookiebot.transfer);
    expect(toolByName('TrustArc')?.pricingFrom).toBe(COMPETITOR_PRICING.TrustArc.pricing);
    expect(toolByName('DataGuard')?.pricingFrom).toBe(COMPETITOR_PRICING.DataGuard.pricing);
    expect(toolByName('Borlabs Cookie')?.pricingFrom).toBe(COMPETITOR_PRICING.BorlabsCookie.pricing);
  });
