export type RuntimeEventKind = 'scan' | 'drift' | 'ai' | 'consent' | 'evidence' | 'incident' | 'agent';
export interface RuntimeEvent { ts: string; kind: RuntimeEventKind; short: string; text: string; target?: string; }
...
export const KIND_LABEL: Record<RuntimeEventKind, string> = { scan: 'scan', drift: 'drift', ai: 'ai', consent: 'consent', evidence: 'evidence', incident: 'incident', agent: 'agent' };
export const KIND_COLOR: Record<RuntimeEventKind, string> = { scan: 'text-cyan-300', drift: 'text-amber-300', ai: 'text-violet-300', consent: 'text-yellow-300', evidence: 'text-emerald-300', incident: 'text-red-300', agent: 'text-titanium-100' };
