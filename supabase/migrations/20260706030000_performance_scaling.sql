...
BEGIN
  ...
  -- Refresh compliance scores materialized view
  REFRESH MATERIALIZED VIEW CONCURRENTLY mv_compliance_scores_by_framework;

  -- Refresh risk heatmap
  REFRESH MATERIALIZED VIEW CONCURRENTLY mv_risk_heatmap;

  -- Refresh task backlog health
  REFRESH MATERIALIZED VIEW CONCURRENTLY mv_task_backlog_health;

  v_end_time := now();

  RETURN QUERY SELECT 3::INT, EXTRACT(EPOCH FROM (v_end_time - v_start_time))::INT * 1000;
END;
$$;
