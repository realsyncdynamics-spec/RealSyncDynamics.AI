-- ═══════════════════════════════════════════════════════════════════════════
--  Kanonischer Plan-Katalog — Neuausgabe für die Add-on-Buchung (AP5–AP8)
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Ersetzt die Katalog-Daten aus 20260902000100_canonical_plan_catalog.sql.
...
-- dieser Lauf schreibt ausschließlich Katalog-Daten.
--
-- Neu gegenüber 20260902000100:
--   - `plan_addons.grants` und `per_unit` je Add-on (Spiegel von
--     `AddOn.grants` in shared/pricing.ts)
--   - eine `products`-Zeile je Add-on (Sentinel `internal_addon_<id>`, kein
--     default_for_plan_key) samt `product_entitlements` — das Produkt, auf
--     das ein Add-on-Grant verweist
--   - `plan_addons.product_id` wird darauf gesetzt
--   - `entitlement_dependencies` aus ENTITLEMENT_DEPENDENCIES
--
-- Nicht enthalten, weil nicht aus dem Repo entscheidbar: `stripe_price_id`
-- je Add-on. Bleibt NULL, bis der Betreiber die Prices angelegt hat
-- (docs/product/addon-booking.md §3). Bis dahin ist kein Add-on buchbar,
