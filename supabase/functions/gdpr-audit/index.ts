...
// Top-Level-Guard: Ein ungefangener Fehler im Handler wird von der
// Edge-Runtime mit einer nackten 500-Text-Antwort **ohne** CORS-Header
// beantwortet. Im Browser scheitert dann bereits die CORS-Prüfung, und der
// Aufrufer sieht nur „Failed to fetch" statt der echten Ursache. Deshalb wird
// hier jeder Fehler abgefangen und als CORS-fähige JSON-Antwort ausgeliefert.
Deno.serve(async (req) => {
  try {
    return await handleAudit(req);
  } catch (e) {
    console.error('gdpr-audit: unhandled error', e);
    return jsonError(500, 'INTERNAL', (e as Error)?.message ?? 'unerwarteter Fehler');
  }
});
