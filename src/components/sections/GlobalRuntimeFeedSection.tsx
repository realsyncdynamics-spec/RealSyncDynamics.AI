                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-px bg-titanium-900">
                        {/* Terminal feed */}
                                ...
                                <motion.div
                                                  ...
                                                  >
                                  {/* HH:MM:SS timestamp */}
                                                  <span className="text-titanium-600 shrink-0 tabular-nums">[{e.ts}]</span>span>
                                  {/* kind badge */}
                                  ...
                                  {/* rule_id → severity */}
                                                  <span className="text-titanium-300 min-w-0 truncate">
                                                                      <span className="text-titanium-100">{e.rule_id}</span>span>
                                                                      <span className="text-titanium-600 mx-1">→</span>span>
                                                                      <span className={SEVERITY_COLOR[e.severity]}>{e.severity}</span>span>
                                                    {e.detail && (
                                                                          <span className="ml-1.5 text-titanium-500">· {e.detail}</span>span>
                                                                      )}
                                                  </span>span>
                                ...
                                <div className="mt-1 text-titanium-600">
                                            ...
                                            <div className="px-4 py-2 border-t border-titanium-900 bg-obsidian-900 font-mono text-[10px] text-titanium-500 flex items-center justify-between">
                                                          <span>{shown.length} / {RUNTIME_MOCK_EVENTS.length} events</span>span>
                                                          <span className="text-emerald-400">● chain sealed</span>span>
                                            </div>div>
                                </div>div>
...
function CounterRow({ label, value, color }: { label: string; value: number; color: string }) {
    return (
          <div className="flex items-center justify-between px-4 py-4">
                <span className="font-mono text-[11px] uppercase tracking-wider text-titanium-400">{label}</span>span>
                <span className={`font-display font-semibold text-2xl tabular-nums ${color}`}>{value}</span>span>
          </div>div>
        );
}</section>
