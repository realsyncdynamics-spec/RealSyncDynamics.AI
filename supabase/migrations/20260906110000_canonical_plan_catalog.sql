-- ═══════════════════════════════════════════════════════════════════════════
--  Kanonischer Plan-Katalog — Neuausgabe: Enterprise verweist auf Growth
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Ersetzt die Katalog-Daten aus 20260904000300_canonical_plan_catalog.sql.
...
--
-- Neu gegenüber 20260904000300 (Freigabe des Eigentümers vom 2026-09-06,
-- Frage 1 der Drei-Fragen-Regel, CLAUDE.md §10.4): Die Feature-Liste von
-- Enterprise begann mit „Alles aus Agency". Agency trägt seit AP2
-- `availability: 'legacy'` und kommt auf keiner Verkaufsfläche mehr vor —
-- ein Interessent las damit einen Verweis auf einen Plan, den er nirgends
-- nachschlagen kann. Die Zeile nennt jetzt „Alles aus Growth", die Stufe,
-- die auf derselben Seite tatsächlich sichtbar ist.
--
-- Inhaltlich bleibt die Aussage richtig: Enterprise enthält Agency, und
-- Agency enthält Growth. Es ändert sich ausschliesslich Anzeigetext in
-- `plan_catalog.features`; Preise, Kontingente, Module, Berechtigungen und
-- Entitlements sind unverändert. Bestandsverträge auf Agency behalten
-- Produkte, Preise und Entitlements vollständig (§7 „Preise, Pläne und
-- Berechtigungen").
--
